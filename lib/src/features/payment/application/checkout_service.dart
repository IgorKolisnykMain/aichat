...
import 'package:aichat/src/features/payment/data/checkout_sessions_repository.dart';
import 'package:aichat/src/features/payment/data/payment_sheet_repository.dart';
...
class CheckoutService {
  ...
  Future<void> pay({
    ...
    final paymentAmount = (product.price * 100).round();
  ...
  Future<void> _showPaymentSheet({
    ...
    try {
      await _ref
          .read(paymentSheetRepositoryProvider)
          .initPaymentSheet(
            email: user.email!,
            amount: amount,
      ...
      final success = await _ref.read(paymentSheetRepositoryProvider).presentPaymentSheet();
      if (success) {
        ...
        _paymentSubscription = _ref.read(paymentsRepositoryProvider).watchPayment(user.uid, paymentIntent).listen((
          payment,
        ) {
          if (payment != null) {
